...
async fn test_same_column_routing() {
    ...
    network.send(TileId(0x00), TileId(0x03), &[0xAB, 0xCD]).await.unwrap();
...
async fn test_cross_column_routing() {
    ...
    network.send(TileId(0x00), TileId(0x50), &[0x12, 0x34, 0x56, 0x78]).await.unwrap();
...
async fn test_dimension_order_routing() {
    ...
    network.send(TileId::new(0, 0).unwrap(), TileId::new(5, 3).unwrap(), &[0xAA]).await.unwrap();
