...
//!
//! Central hub connecting quadrants in the Newport RaceWay.
//! Based on Hub.v - routes between left/right halves and columns.

use crate::packet::{Command, RaceWayPacket, TileId};
use crate::broadcast::{BroadcastManager, BroadcastDomain};
...
pub struct Hub {
    ...
    center_channels: HashMap<u8, (mpsc::UnboundedSender<RaceWayPacket>, mpsc::UnboundedReceiver<RaceWayPacket>)>,
    ...
    pub fn route(&mut self, packet: RaceWayPacket) -> Result<()> {
        ...
        if packet.is_broadcast() {
            ...
            if dest_quadrant < 2 && source_quadrant >= 2 {
                // South to North
                self.west_tx.send(packet).map_err(|_| RaceWayError::ChannelFull)?;
            } else if dest_quadrant >= 2 && source_quadrant < 2 {
                // North to South
                self.east_tx.send(packet).map_err(|_| RaceWayError::ChannelFull)?;
...
mod tests {
    ...
    async fn test_hub_routing() {
        let (west_tx, west_rx) = mpsc::unbounded_channel();
        let (hub_west_tx, hub_west_rx) = mpsc::unbounded_channel();
        let (east_tx, east_rx) = mpsc::unbounded_channel();
        let (hub_east_tx, hub_east_rx) = mpsc::unbounded_channel();
